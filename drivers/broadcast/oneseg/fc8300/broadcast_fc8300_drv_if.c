...
static void broadcast_drv_if_get_oneseg_sig_info(fc8300Status_t *pst, struct broadcast_dmb_control_info *pInfo, s32 brd_type)
{
	int antlvl;
	int ber;
	int per;
	...
	switch(pst->antenna_level)
	{
		case 0:
			if((ber < 500) && (cn >= 1600))
				antlvl = pst->antenna_level = 1;
			else
				antlvl = pst->antenna_level;
		break;
		case 1:
			if((ber > 550) && (cn < 1700))
				antlvl = pst->antenna_level = 0;
			if((ber < 400) || (cn > 2000))
				antlvl = pst->antenna_level = 2;
			else
				antlvl = pst->antenna_level;
		break;
		case 2:
	...
	pInfo->sig_info.info.oneseg_info.antenna_level = antlvl;
...
int	broadcast_drv_if_get_sig_info(struct broadcast_dmb_control_info *pInfo)
{
	...
	if((TimeCount_ms() - fcTimer) > 500) {
		ret = tunerbb_drv_fc8300_Get_SignalInfo(&st, broad_type);
		setTimer();
		}
	...
	case ENUM_GET_ONESEG_SIG_INFO:
		broadcast_drv_if_get_oneseg_sig_info(&st, pInfo, broad_type);
	break;
...
/*
	if(pdmb_data->data_buf_addr == NULL) {
