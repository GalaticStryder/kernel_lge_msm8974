...
{
	gpio_request(ISDB_EN, "ISDBT_EN");
	udelay(50);
	gpio_direction_output(ISDB_EN, 0);

	gpio_request(ISDB_RESET_N, "ISDBT_RST");
	udelay(50);
	gpio_direction_output(ISDB_RESET_N, 0);
}
...
int tunerbb_drv_fc8300_Get_SignalInfo(fc8300Status_t *st, s32 brd_type)
{
	...
	print_log(NULL, "layA : 0x%x, layB : 0x%x, layC : 0x%x, TMCC : 0x%x , sysinfo : 0x%x \n", st->layerinfo_a, st->layerinfo_b, st->layerinfo_c, st->tmccinfo, st->sysinfo);
	return res;
}
